...
describe('demand', () => {
  ...
  it('lower L1 multiplier lowers demand for every positive-growth sector in every plan year', () => {
    const hi = computeDemand(base(), assumptions, { exportTerminalSelected: false })
  ...
  it('adds no export demand when L5 is domestic', () => {
    const d = computeDemand(withL({ L5: 0 }), assumptions, { exportTerminalSelected: false })
    expect(d.exportKt).toEqual([0, 0, 0, 0, 0, 0])
    expect(d.exportLevel).toBe(0)
  })

  it('adds GCC export potential when L5 is GCC, zero in the base year', () => {
    const d = computeDemand(withL({ L5: 1 }), assumptions, { exportTerminalSelected: false })
  ...
  it('caps extended export at the GCC level until the Jeddah terminal is selected', () => {
    const without = computeDemand(withL({ L5: 2 }), assumptions, { exportTerminalSelected: false })
    const withTerminal = computeDemand(withL({ L5: 2 }), assumptions, {
      exportTerminalSelected: true,
    })
