...
Steps 1 to 5 of the build order in `CLAUDE.md` are complete: shell, the whole engine under `src/engine` behind `runPlan(levers, data)`, All six views are live: Financial plan, Growth portfolio, Strategic direction, Operations and people (year scrubber, site gauges, capex sparklines, people and supply chain panels) Roadmap (quarterly Gantt in the five layers with ghost bars, dependency lines, critical path and milestones), and Tracker (2027 actuals for demand, energy and carbon re-run the engine; the triggers-fired panel lists the decisions now due). View 1 carries the Strata reveal: three bands (assumptions, initiatives, plan) that light top to bottom with connector lines when a lever or a chip is hovered, the Where-to-play 2x2 with animated bubbles, and the classification table with the change against Base. 157 tests.
